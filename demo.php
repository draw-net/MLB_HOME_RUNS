...
function calculateBallWeight(speed, angle, distance) {
    ...
    return effectiveWeight.toFixed(4);
...
function visualizeTrajectory(homeRunID, distance, exitVelocity, launchAngle, videoUrl) {
    ...
    const deformedMaterial = new THREE.MeshBasicMaterial({ color: 0xff0000 }); // Red

    // Create deformed ball
    const deformedBall = new THREE.Mesh(geometry, deformedMaterial);
    ...
    // rotation speed calc
    const rotationSpeed = 0.1 * parseFloat(exitVelocity) / 100;

    let fullRotationCompleted = false; // Track if the animation finished
    let totalRotation = 0;

        function animate() {
                if (!fullRotationCompleted) {
                    deformedBall.rotation.y += rotationSpeed;
    ...
    ballWeightLabel.textContent = `Peso da Bola: ${ballWeight} kg`;
}
